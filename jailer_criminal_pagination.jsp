...
<%
    ...
    String sqlPagination="SELECT SQL_CALC_FOUND_ROWS * FROM criminal limit "+iPageNo+","+iShowRows+"";
...
%>
    ...
    <tr>
		...
		<td><%= crmob %></td>
		<%  
		out.println("<td><a href='#' onClick=\"modify(\'"+cri+"\', \'"+crn+"\', \'"+cd1+"\', \'"+cd2+"\', \'"+cra+"\', \'"+crdob+"\', \'"+crmob+"\')\">Modify</a>	</td>");
		out.println("<td><a href=\"del_criminal.jsp?crID="+ rsPagination.getString("C_ID") + "\">Delete</a></td>");
		%>
    </tr>
    <% 
 }
...
<%
        ...
        {
         %>
          <a href="jailer_criminal_record.jsp?iPageNo=<%=prePageNo%>&cPageNo=<%=prePageNo%>"> << Previous</a>
         <%
        }
        ...
        for(i=((cPage*iTotalSearchRecords)-(iTotalSearchRecords-1));i<=(cPage*iTotalSearchRecords);i++)
        {
          ...
          {
          %>
           <a href="jailer_criminal_record.jsp?iPageNo=<%=i%>" style="cursor:pointer;color:#DAA520"><b><%=i%></b></a>
          <%
          }
          else if(i<=iTotalPages)
          {
          %>
           <a href="jailer_criminal_record.jsp?iPageNo=<%=i%>"><%=i%></a>
          <% 
          }
        }
        ...
        {
         %>
         <a href="jailer_criminal_record.jsp?iPageNo=<%=i%>&cPageNo=<%=i%>"> >> Next</a> 
         <%
        }
        }
      %>
