...
<%
    ...
    String sqlPagination="SELECT SQL_CALC_FOUND_ROWS * FROM criminal,c_Details where criminal.C_ID=c_Details.C_ID and criminal.Mobile like ? limit "+iPageNo+","+iShowRows+"";
...
<table width="100%" border="1" style="margin-bottom:3%">
<tr>
<th>ID</th>
<th>Name</th>
<th>ID 1</th>
...
<%
        ...
        {
         %>
          <a href="searchByNamejsp?iPageNo=<%=prePageNo%>&cPageNo=<%=prePageNo%>"> << Previous</a>
         <%
        }
        ...
        for(i=((cPage*iTotalSearchRecords)-(iTotalSearchRecords-1));i<=(cPage*iTotalSearchRecords);i++)
        {
          ...
          {
          %>
           <a href="searchByName.jsp?iPageNo=<%=i%>" style="cursor:pointer;color:#DAA520"><b><%=i%></b></a>
          <%
          }
          else if(i<=iTotalPages)
          {
          %>
           <a href="searchByName.jsp?iPageNo=<%=i%>"><%=i%></a>
          <% 
          }
        }
        ...
        {
         %>
         <a href="searchByName.jsp?iPageNo=<%=i%>&cPageNo=<%=i%>"> >> Next</a> 
         <%
        }
        }
      %>
