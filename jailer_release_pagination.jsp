...
%>
	<%
		...
		String ard = rsPagination.getString("ARD");
	%>
    <tr>
		<td><%= csid %></td>
		<td><%= csno %></td>
		...
		<td><%= ard %></td>
		<%  
		out.println("<td><a href='#' onClick=\"modify(\'"+csid+"\',\'"+csno+"\', \'"+jlid+"\', \'"+ard+"\')\">Release</a>	</td>");
		%>
    </tr>
    <% 
 }
...
<%
        ...
        {
         %>
          <a href="jailer_release.jsp?iPageNo=<%=prePageNo%>&cPageNo=<%=prePageNo%>"> << Previous</a>
         <%
        }
        ...
        for(i=((cPage*iTotalSearchRecords)-(iTotalSearchRecords-1));i<=(cPage*iTotalSearchRecords);i++)
        {
          ...
          {
          %>
           <a href="jailer_release.jsp?iPageNo=<%=i%>" style="cursor:pointer;color:#DAA520"><b><%=i%></b></a>
          <%
          }
          else if(i<=iTotalPages)
          {
          %>
           <a href="jailer_release.jsp?iPageNo=<%=i%>"><%=i%></a>
          <% 
          }
        }
        ...
        {
         %>
         <a href="jailer_release.jsp?iPageNo=<%=i%>&cPageNo=<%=i%>"> >> Next</a> 
         <%
        }
        }
      %>
